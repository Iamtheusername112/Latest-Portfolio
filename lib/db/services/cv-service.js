...
export class CVService {
  ...
  static async updateCVContent(data) {
    try {
      ...
      // Ensure languages is explicitly set as an array (even if empty)
      // This prevents it from being undefined or null
      if (!Array.isArray(cleanData.languages)) {
        cleanData.languages = []
      }

      // Ensure experience is explicitly set as an array (even if empty)
      // This prevents it from being undefined or null
      ...
      console.log('Updating CV with data:', {
        ...
        languagesCount: cleanData.languages.length,
        experience: cleanData.experience,
        experienceCount: cleanData.experience.length,
        experienceType: typeof cleanData.experience,
        experienceIsArray: Array.isArray(cleanData.experience)
      })
      ...
      if (existing) {
        ...
        console.log('CV updated successfully:', {
          ...
          languagesCount: result[0]?.languages?.length || 0,
          experience: result[0]?.experience,
          experienceCount: result[0]?.experience?.length || 0
        })
