...
export class ProjectsService {
    ...
    try {
      ...
      stats.averageProgress =
        allProjects.length > 0
          ? Math.round(stats.totalProgress / allProjects.length)
          : 0
  ...
  static processProjectData(data) {
    return {
      ...
      techStack:
        data.techStack && typeof data.techStack === 'object'
          ? data.techStack
          : {},
      ...
      estimatedHours: data.estimatedHours
        ? parseInt(data.estimatedHours)
        : null,
      actualHours: data.actualHours ? parseInt(data.actualHours) : null,
      progress: data.progress
        ? Math.max(0, Math.min(100, parseInt(data.progress)))
        : 0,
