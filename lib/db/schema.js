...
export const cvContent = pgTable('cv_content', {
  ...
  // Skills
  technicalSkills: json('technical_skills'), // { frontend: [], backend: [], tools: [], languages: [] }
  softSkills: json('soft_skills'), // []
  
  // Experience
  experience: json('experience'), // [{ title, company, period, description, achievements: [] }]
  
  // Education
  education: json('education'), // [{ degree, school, year, description }]
  
  // Projects
  projects: json('projects'), // [{ name, description, technologies: [], link }]
  
  // Certifications
  certifications: json('certifications'), // [{ name, issuer, date, credentialId }]
  
  // Languages
  languages: json('languages'), // [{ language, proficiency }]
  
  // Additional Information
  additionalInfo: json('additional_info'), // { interests: [], volunteer: [], publications: [] }
  
  // CV Settings
  template: varchar('template', { length: 50 }).default('modern'), // modern, classic, creative
  colorScheme: varchar('color_scheme', { length: 50 }).default('blue'), // blue, green, purple, red, gray
  layout: varchar('layout', { length: 50 }).default('single'), // single, two-column
  showPhoto: boolean('show_photo').default(false),
